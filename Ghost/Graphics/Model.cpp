...
namespace graphics
{
	...
	bool Model::MakeVBO(DeviceInfo* deviceInfo)
	{
		...
		m_vertexBuffer.Allocate(m_deviceInfo->PhysicalDevice, m_deviceInfo->Device, m_vertices.data(), buffer_size, 
			VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
		
		buffer_size = (int)(sizeof(unsigned int) * m_indices.size());

		m_IndexBuffer.Allocate(m_deviceInfo->PhysicalDevice, m_deviceInfo->Device, m_indices.data(), buffer_size,
	...
	void Model::DrawIndexed(VkCommandBuffer cmdBuffer, VkPipelineLayout layout)
	{
		...
		vkCmdPushConstants(cmdBuffer, layout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(glm::mat4), &modelMatrix);
