...
namespace graphics
{
	void Renderer::Init(DeviceInfo* deviceInfo, OffscreenData* offScreenData)
	{
	...
	void Renderer::DrawFrame(std::vector<RenderModel>& renderModels, ui::UI* gui)
	{
		...
		RecordCommandBuffer(m_deviceInfo->_CommandBuffers[currentFrame], m_deviceInfo->pipelineLayout, imageIndex, renderModels, gui);
	...
	void Renderer::RecordCommandBuffer(VkCommandBuffer cmdBuffer, VkPipelineLayout layout, uint32_t imageIndex, std::vector<RenderModel>& renderModels, ui::UI* gui)
	{
	...
	void Renderer::Re_CreateSwapchain()
	{
		...
		*/
		...
		{
			std::array<VkImageView, 2> attachments = { m_deviceInfo->swapchainImageView[i], depthView };
	...
	void Renderer::CreateDepthResource()
	{
		...
		//Image Creation
		initializer::createImage(d_width, d_height, depthFormat, VK_SAMPLE_COUNT_1_BIT, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT,
	...
	{
		VkPipelineShaderStageCreateInfo vertex_stage = createshaderStage( getProjectPath() + "/Ghost/Asset/scene_vert.spv", VK_SHADER_STAGE_VERTEX_BIT, m_deviceInfo->Device);
		VkPipelineShaderStageCreateInfo fragment_stage = createshaderStage(getProjectPath() + "/Ghost/Asset/scene_frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT, m_deviceInfo->Device);
		...
		VkPipelineMultisampleStateCreateInfo multisampleCI = initializer::MultiSampleStateInfo(VK_SAMPLE_COUNT_1_BIT);
		...
		shaderstage[0] = createshaderStage(getProjectPath() + "/Ghost/Asset/quad_vert.spv", VK_SHADER_STAGE_VERTEX_BIT, m_deviceInfo->Device);
		shaderstage[1] = createshaderStage(getProjectPath() + "/Ghost/Asset/quad_frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT, m_deviceInfo->Device);
		...
		VK_CHECK_RESULT(vkCreateGraphicsPipelines(m_deviceInfo->Device, nullptr, 1, &pipelineInfo, nullptr, &m_offScreenInfo->debugPipeline));
		...
		VK_CHECK_RESULT(vkCreateGraphicsPipelines(m_deviceInfo->Device, nullptr, 1, &pipelineInfo, nullptr, &m_deviceInfo->pipeline));
		...
		VkPipelineShaderStageCreateInfo offscreenVertexShader = createshaderStage(getProjectPath() + "/Ghost/Asset/OffScreen_vert.spv", VK_SHADER_STAGE_VERTEX_BIT, m_deviceInfo->Device);
		...
		VK_CHECK_RESULT(vkCreateGraphicsPipelines(m_deviceInfo->Device, nullptr, 1, &pipelineInfo, nullptr, &m_offScreenInfo->pipeline));
