...
namespace graphics
{
	void GPUDevice::Init(DeviceInfo* deviceInfo)
	{
		...
		std::set<uint32_t> uniqueQueueFamilies = { indices.graphicsQueueIndex.value()};
	...
	VkSurfaceFormatKHR GPUDevice::chooseswapsurfaceFormat(const std::vector<VkSurfaceFormatKHR>& availableFormat)
	{
